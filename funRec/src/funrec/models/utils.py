...
def add_tensor_func(tensor_list, name=None):
    ...
    if len(tensor_list) == 1:
        return (
            tensor_list[0]
            if name is None
            else tf.keras.layers.Lambda(lambda x: x, name=name)(tensor_list[0])
        )
    else:
...
def get_linear_logits(
    ...
    # 使用 Lambda 层替代 Identity 层（兼容性更好）
    linear_logits = tf.keras.layers.Lambda(lambda x: x, name=name)(linear_logits)
...
def get_cross_logits(
    ...
    # 使用 Lambda 层替代 Identity 层（兼容性更好）
    cross_logits = tf.keras.layers.Lambda(lambda x: x, name=name)(cross_logits)
    return cross_logits
