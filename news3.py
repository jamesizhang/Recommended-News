...
def usercf_sim(all_click_df, user_activate_degree_dict, batch_size=1000, min_sim_threshold=0.0):
    """
        用户相似性矩阵计算（优化版本：分批处理避免OOM）
        :param all_click_df: 数据表
        :param user_activate_degree_dict: 用户活跃度的字典
        :param batch_size: 分批处理的批次大小，默认1000，如果内存不足可以减小
        :param min_sim_threshold: 最小相似度阈值，低于此值的用户对不保存（默认0.0，保存所有）
        return 用户相似性矩阵

        思路: 基于用户的协同过滤(详细请参考上一期推荐系统基础的组队学习) + 关联规则
        优化: 分批处理items，避免一次性加载所有数据导致OOM
    """
    item_user_time_dict = get_item_user_time_dict(all_click_df)

    u2u_sim = collections.defaultdict(lambda: collections.defaultdict(float))
    user_cnt = defaultdict(int)
    
    # 检查是否已有部分结果（断点续传）
    checkpoint_file = save_path + 'usercf_u2u_sim_checkpoint.pkl'
    processed_items = set()
    if os.path.exists(checkpoint_file):
        print(f"发现检查点文件，尝试恢复进度...")
        try:
            with open(checkpoint_file, 'rb') as f:
                checkpoint_data = pickle.load(f)
            # 从普通 dict 恢复为 defaultdict
            u2u_sim_dict = checkpoint_data.get('u2u_sim', {})
            u2u_sim = collections.defaultdict(lambda: collections.defaultdict(float))
            for u, related_users in u2u_sim_dict.items():
                u2u_sim[u] = collections.defaultdict(float, related_users)
            
            user_cnt_dict = checkpoint_data.get('user_cnt', {})
            user_cnt = defaultdict(int, user_cnt_dict)
            
            processed_items = checkpoint_data.get('processed_items', set())
            print(f"已处理 {len(processed_items)} 个 items，继续处理...")
        except Exception as e:
            print(f"检查点文件损坏，从头开始处理... 错误: {e}")
            processed_items = set()
    
    # 获取所有 items 并过滤已处理的
    all_items = list(item_user_time_dict.items())
    remaining_items = [(item, user_time_list) for item, user_time_list in all_items if item not in processed_items]
    
    print(f"总 items 数: {len(all_items)}, 已处理: {len(processed_items)}, 待处理: {len(remaining_items)}")
    
    # 分批处理 items
    for batch_idx in tqdm(range(0, len(remaining_items), batch_size), desc="处理 items 批次"):
        batch_items = remaining_items[batch_idx:batch_idx + batch_size]
        
        # 处理当前批次的 items
        for item, user_time_list in batch_items:
            # 只处理用户数大于1的 item（否则无法计算用户相似度）
            if len(user_time_list) < 2:
                continue
            
            # 优化1: 预先计算公共值，避免在循环内重复计算
            log_item_users = math.log(len(user_time_list) + 1)
            
            # 优化2: 提取用户列表，避免重复访问
            users = [u for u, _ in user_time_list]
            
            # 优化3: 只计算上三角矩阵，避免重复计算 (u,v) 和 (v,u)
            # 但注意：原逻辑中 (u,v) 和 (v,u) 的增量可能不同（因为活跃度权重不同）
            # 所以这里保持原逻辑，但优化循环结构
            for i, (u, click_time) in enumerate(user_time_list):
                user_cnt[u] += 1
                u2u_sim.setdefault(u, {})
                
                # 只遍历后面的用户，避免重复计算自己
                for j in range(i + 1, len(user_time_list)):
                    v, click_time_v = user_time_list[j]
                    u2u_sim[u].setdefault(v, 0)
                    u2u_sim[v].setdefault(u, 0)
                    
                    # 用户平均活跃度作为活跃度的权重
                    activate_weight = 100 * 0.5 * (user_activate_degree_dict[u] + user_activate_degree_dict[v])
                    sim_increment = activate_weight / log_item_users
                    
                    # 同时更新 (u,v) 和 (v,u)，因为相似度是对称的
                    u2u_sim[u][v] += sim_increment
                    u2u_sim[v][u] += sim_increment
        ...
        if (batch_idx // batch_size) % 10 == 0:
            checkpoint_data = {
                'u2u_sim': dict(u2u_sim),  # 转换为普通 dict 以便序列化
    ...
    print("开始归一化相似度矩阵...")
    # 归一化相似度矩阵（直接在原字典上修改，避免复制）
    # 先转换为普通 dict 以便后续处理
    u2u_sim_normalized = {}
    for u, related_users in tqdm(u2u_sim.items(), desc="归一化相似度"):
        if u not in u2u_sim_normalized:
            u2u_sim_normalized[u] = {}
        for v, wij in related_users.items():
            if user_cnt[u] > 0 and user_cnt[v] > 0:
                normalized_sim = wij / math.sqrt(user_cnt[u] * user_cnt[v])
                # 只保存大于阈值的相似度（如果设置了阈值）
                if normalized_sim >= min_sim_threshold:
                    u2u_sim_normalized[u][v] = normalized_sim
    ...
    with open(save_path + 'usercf_u2u_sim.pkl', 'wb') as f:
        pickle.dump(u2u_sim_normalized, f)
    ...
    print(f"用户相似度矩阵计算完成，共 {len(u2u_sim_normalized)} 个用户")
...
def embdding_sim(click_df, item_emb_df, save_path, topk, batch_size=1000):
    ...
    if os.path.exists(checkpoint_file):
        ...
        try:
            with open(checkpoint_file, 'rb') as f:
                checkpoint_data = pickle.load(f)
            item_sim_dict = checkpoint_data.get('item_sim_dict', collections.defaultdict(dict))
            start_idx = checkpoint_data.get('last_idx', 0)
            print(f"从索引 {start_idx} 继续处理...")
        except:
            print("检查点文件损坏，从头开始处理...")
            start_idx = 0
    
    # 分批处理
    for batch_start in tqdm(range(start_idx, n_items, batch_size), desc="处理批次"):
...
def youtubednn_u2i_dict(data, topk=20):
    ...
    # 类别编码
    # 注意：如果在采样数据上训练，只能对采样中的用户/物品进行召回
    # 对于新用户/新物品（OOV），需要使用全量数据训练或采用其他策略
    ...
    print(f"注意：如果在采样数据上训练，只能对采样中的用户/物品进行召回")
    print(f"对于新用户/新物品，建议使用全量数据训练或采用 OOV 处理策略")
    print(f"工业界最佳实践：使用全量数据（或至少大部分数据）训练 LabelEncoder，确保覆盖大部分用户/物品")
    ...
    def get_feature_data(input_layer, input_dict):
        ...
        # click_article_id 应该是 (batch_size, 1) 的形状
...
# 检查 emb_i2i_sim 是否存在，如果不存在则重新计算
if os.path.exists(save_path + 'emb_i2i_sim.pkl'):
    emb_i2i_sim = pickle.load(open(save_path + 'emb_i2i_sim.pkl', 'rb'))
else:
    print("emb_i2i_sim.pkl 不存在，开始计算...")
    item_emb_df = pd.read_csv(data_path + 'articles_emb.csv')
    emb_i2i_sim = embdding_sim(all_click_df, item_emb_df, save_path, topk=50, batch_size=500)  # 使用较小的batch_size避免OOM
...
def u2u_embdding_sim(click_df, user_emb_dict, save_path, topk, batch_size=1000):
    """
        ...
        :param click_df: 数据表
        :param user_emb_dict: 用户的embedding字典
        :param save_path: 保存路径
        :param topk: 找最相似的topk个用户
        :param batch_size: 分批处理的批次大小，默认1000，如果内存不足可以减小
        return 用户相似性矩阵
    """
    user_list = []
    ...
    if os.path.exists(checkpoint_file):
        ...
        try:
            with open(checkpoint_file, 'rb') as f:
                checkpoint_data = pickle.load(f)
            user_sim_dict = checkpoint_data.get('user_sim_dict', collections.defaultdict(dict))
            start_idx = checkpoint_data.get('last_idx', 0)
            print(f"从索引 {start_idx} 继续处理...")
        except:
            print("检查点文件损坏，从头开始处理...")
    ...
    # 分批处理
    for batch_start in tqdm(range(start_idx, n_users, batch_size), desc="处理用户批次"):
        ...
        for local_idx, (sim_value_list, rele_idx_list) in enumerate(zip(sim, idx)):
            ...
            try:
                ...
                print(f"KeyError: {e}, target_idx={target_idx}, rele_idx={rele_idx}")
