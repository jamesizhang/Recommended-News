"""
本地可修改版本 - 支持多个 item features
Author:
    Weichen Shen, weichenswc@163.com (原始版本)
    修改：支持多个 item features
Reference:
...
def YoutubeDNN(user_feature_columns, item_feature_columns,
    ...
    修改：支持多个 item features（原始版本只支持 1 个）

    :param user_feature_columns: An iterable containing user's features used by  the model.
    :param item_feature_columns: An iterable containing item's features used by  the model. (现在支持多个)
    ...
    # 修改：支持多个 item features
    # 如果有多个 item features，将它们 concat 起来
    if len(item_feature_columns) == 1:
        # 单个 item feature（原始逻辑）
        item_index = EmbeddingIndex(list(range(item_vocabulary_size)))(item_features[item_feature_name])
        item_embedding_matrix = embedding_matrix_dict[item_feature_name]
        item_embedding_weight = NoMask()(item_embedding_matrix(item_index))
        pooling_item_embedding_weight = PoolingLayer()([item_embedding_weight])
    else:
        # 多个 item features：分别获取 embedding 然后 concat
        item_embedding_list = []
        for item_feat_col in item_feature_columns:
            feat_name = item_feat_col.name
            feat_vocab_size = item_feat_col.vocabulary_size
            item_index = EmbeddingIndex(list(range(feat_vocab_size)))(item_features[feat_name])
            item_embedding_matrix = embedding_matrix_dict[feat_name]
            item_embedding_weight = NoMask()(item_embedding_matrix(item_index))
            item_embedding_list.append(item_embedding_weight)
        
        # 将多个 item feature embeddings concat 起来
        from deepctr.layers.utils import concat_func
        pooling_item_embedding_weight = concat_func(item_embedding_list)
