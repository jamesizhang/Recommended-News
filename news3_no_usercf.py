...
# 定义一个多路召回的字典，将各路召回的结果都保存在这个字典当中（已移除UserCF相关召回）
...
# ========== UserCF相关函数已移除 ==========
# 以下函数已删除：
# - get_user_activate_degree_dict: 用户活跃度计算
# - usercf_sim: 用户相似度矩阵计算
# - user_based_recommend: 基于用户的召回
# - u2u_embdding_sim: 基于embedding的用户相似度计算
...
def youtubednn_u2i_dict(data, topk=20):
    ...
    if gpus:
        try:
            # 设置GPU内存增长，而不是一次性分配所有内存
            for gpu in gpus:
                tf.config.experimental.set_memory_growth(gpu, True)
            print(f"已设置GPU内存增长模式，GPU数量: {len(gpus)}")
    ...
    # 减小batch_size以避免GPU内存溢出（OOM）
    # 如果仍然OOM，可以进一步减小到32或16
    batch_size = 128  # 从256减小到128，如果仍然OOM可以减小到64或32
    print(f"使用batch_size={batch_size}进行训练，如果仍然OOM，请减小batch_size")
    history = model.fit(train_model_input, train_label, batch_size=batch_size, epochs=5, verbose=1, validation_split=0.1)
...
# ========== UserCF相关代码已移除 ==========
# 已删除：
# - user_based_recommend 函数
# - UserCF召回代码块
# - u2u_embdding_sim 函数
# - YouTubeDNN+UserCF召回代码块
