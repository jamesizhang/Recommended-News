...
# all_click_df = get_all_click_df(offline=False, sample_size=500000)  # 如果内存不足，可以采样
...
def get_user_item_time(click_df):
    print(f"\n=== get_user_item_time 函数调试 ===")
    print(f"输入 click_df 形状: {click_df.shape}")
    print(f"输入 click_df 中的 user_id 唯一值数量: {click_df['user_id'].nunique()}")
    print(f"输入 click_df 中的 user_id 示例（前5个）: {click_df['user_id'].unique()[:5]}")
    print(f"输入 click_df 中的 user_id 最小值: {click_df['user_id'].min()}")
    print(f"输入 click_df 中的 user_id 最大值: {click_df['user_id'].max()}")
    print(f"输入 click_df 中的 user_id 是否包含 0: {0 in click_df['user_id'].values}")
    print(f"输入 click_df 的索引范围: {click_df.index.min()} - {click_df.index.max()}")
    
    click_df = click_df.sort_values('click_timestamp')
    
    print(f"\n排序后的 click_df:")
    print(f"   user_id 列的最小值: {click_df['user_id'].min()}")
    print(f"   user_id 列的最大值: {click_df['user_id'].max()}")
    print(f"   user_id 列是否包含 0: {0 in click_df['user_id'].values}")
    print(f"   索引范围: {click_df.index.min()} - {click_df.index.max()}")

    def make_item_time_pair(df):
        return list(zip(df['click_article_id'], df['click_timestamp']))

    # 先执行 groupby，保存结果以便调试
    grouped_result = click_df.groupby('user_id')[['click_article_id', 'click_timestamp']].apply(
        lambda x: make_item_time_pair(x))
    
    print(f"\ngroupby 后的结果:")
    print(f"   索引类型: {type(grouped_result.index)}")
    print(f"   索引名称: {grouped_result.index.name}")
    print(f"   索引示例（前5个）: {grouped_result.index[:5].tolist()}")
    print(f"   索引的最小值: {grouped_result.index.min()}")
    print(f"   索引的最大值: {grouped_result.index.max()}")
    
    if grouped_result.index.name == 'user_id':
        if grouped_result.index.min() == 0 and grouped_result.index.max() < 100:
            print(f"   ⚠️ 问题：groupby 后的索引是 [0, 1, 2, 3, 4...]，这不是 user_id 的值！")
            print(f"   这说明 groupby 可能使用了错误的索引或数据！")
        else:
            print(f"   ✓ groupby 后的索引是正确的（user_id 的值）")
    else:
        print(f"   ⚠️ 索引名称不是 user_id，这是问题所在！")
    
    user_item_time_df = grouped_result.reset_index().rename(columns={0: 'item_time_list'})
    
    print(f"\ngroupby + reset_index 后的 user_item_time_df 形状: {user_item_time_df.shape}")
    print(f"user_item_time_df 的列名: {user_item_time_df.columns.tolist()}")
    
    print(f"\nuser_item_time_df 的前5行:")
    print(user_item_time_df.head())
    print(f"user_item_time_df 的 user_id 列示例（前5个）: {user_item_time_df['user_id'].head().tolist()}")
    print(f"user_item_time_df 的索引范围: {user_item_time_df.index.min()} - {user_item_time_df.index.max()}")
    
    # 检查 user_id 32
    if 32 in user_item_time_df['user_id'].values:
        user_32_row = user_item_time_df[user_item_time_df['user_id'] == 32]
        print(f"\nuser_id 32 在 user_item_time_df 中:")
        print(user_32_row)
        print(f"user_id 32 的 item_time_list: {user_32_row['item_time_list'].iloc[0]}")
        print(f"user_id 32 所在的行索引: {user_32_row.index[0]}")
    else:
        print(f"\n⚠️ user_id 32 不在 user_item_time_df 中！")
        print(f"user_item_time_df 中的 user_id 示例（前10个）: {user_item_time_df['user_id'].head(10).tolist()}")
    
    user_item_time_dict = dict(zip(user_item_time_df['user_id'], user_item_time_df['item_time_list']))
    
    print(f"\n生成的 user_item_time_dict 长度: {len(user_item_time_dict)}")
    if 32 in user_item_time_dict:
        print(f"user_id 32 在 user_item_time_dict 中: {user_item_time_dict[32]}")
    
    print("=== get_user_item_time 函数调试结束 ===\n")
...
def get_hist_and_last_click(all_click):
    print(f"\n=== get_hist_and_last_click 函数调试 ===")
    print(f"输入 all_click 形状: {all_click.shape}")
    ...
    print(f"click_hist_df 形状: {click_hist_df.shape}")
    print(f"click_last_df 形状: {click_last_df.shape}")
    
    # 检查 user_id 32
    if 32 in all_click['user_id'].values:
        user_32_all = all_click[all_click['user_id'] == 32]
        user_32_hist = click_hist_df[click_hist_df['user_id'] == 32]
        user_32_last = click_last_df[click_last_df['user_id'] == 32]
        
        print(f"\nuser_id 32 的点击记录:")
        print(f"  全部点击记录数: {len(user_32_all)}")
        print(f"  全部点击记录: {user_32_all[['user_id', 'click_article_id', 'click_timestamp']].values}")
        print(f"  历史点击记录数: {len(user_32_hist)}")
        if len(user_32_hist) > 0:
            print(f"  历史点击记录: {user_32_hist[['user_id', 'click_article_id', 'click_timestamp']].values}")
        print(f"  最后一次点击记录数: {len(user_32_last)}")
        if len(user_32_last) > 0:
            print(f"  最后一次点击记录: {user_32_last[['user_id', 'click_article_id', 'click_timestamp']].values}")
    
    print("=== get_hist_and_last_click 函数调试结束 ===\n")
...
# 现在支持传递辅助特征（用户特征和物品特征）
def gen_data_set(data, negsample=0, user_feature_cols=None, item_feature_cols=None):
    """
    Args:
        data: 包含 user_id, click_article_id, click_timestamp 以及可选的辅助特征
        negsample: 负采样数量
        user_feature_cols: 用户特征列名列表（如 ['click_environment', 'click_os']）
        item_feature_cols: 物品特征列名列表（如 ['category_id', 'words_count']）
    """
        ...
        for i in range(1, len(pos_list)):
            ...
            if i != len(pos_list) - 1:
                train_set.append((reviewerID, hist[::-1], pos_list[i], 1,
                                  len(hist[::-1]), user_features.copy(), item_features.copy()))  # 正样本 [user_id, his_item, pos_item, label, len(his_item), user_features, item_features]
                for negi in range(negsample):
                    ...
                    train_set.append((reviewerID, hist[::-1], neg_list[i * negsample + negi], 0,
                                      len(hist[::-1]), user_features.copy(), neg_item_features.copy()))  # 负样本 [user_id, his_item, neg_item, label, len(his_item), user_features, item_features]
            else:
                # 将最长的那一个序列长度作为测试数据
                test_set.append((reviewerID, hist[::-1], pos_list[i], 1, len(hist[::-1]), user_features.copy(), item_features.copy()))
...
def gen_model_input(train_set, user_profile, seq_max_len, user_feature_cols=None, item_feature_cols=None):
    ...
    if len(train_set) > 0 and len(train_set[0]) > 6:  # 有辅助特征
        if user_feature_cols:
...
def youtubednn_u2i_dict(data, item_info_df=None, all_click_df=None, topk=20, use_aux_features=True):
    """
    YouTubeDNN 召回函数（双塔模型）
    
    Args:
        data: 点击数据，需要包含 user_id, click_article_id, click_timestamp
        item_info_df: 物品信息表，包含 category_id, words_count, created_at_ts 等（可选）
        all_click_df: 全量点击数据，用于获取用户特征如 click_environment, click_os 等（可选）
        topk: 召回数量
        use_aux_features: 是否使用辅助特征（用户特征和物品特征）来帮助处理冷启动
    """
    ...
    # 定义用户特征和物品特征（用于双塔模型，保持独立）
    # 用户特征：来自点击日志，用于区分不同用户（特别是新用户）
    user_aux_features = []
    if use_aux_features and all_click_df is not None:
        # 从全量点击数据中获取用户特征（使用众数，因为一个用户可能有多个值）
        user_feature_cols = ['click_environment', 'click_deviceGroup', 'click_os', 
                           'click_country', 'click_region', 'click_referrer_type']
        # 检查 all_click_df 是否包含这些列
        available_feature_cols = [col for col in user_feature_cols if col in all_click_df.columns]
        if len(available_feature_cols) == 0:
            print("Warning: all_click_df does not contain user feature columns. Skipping user features.")
        else:
            # 只保留在 data 中存在的用户
            ...
            if len(user_features_df) > 0:
                ...
                for col in available_feature_cols:
                    if col in user_features_df.columns:
                        ...
                        # 如果 data 中已经有这个列，先删除避免冲突
                        if col in data.columns:
                            data = data.drop(columns=[col])
                        data = data.merge(user_mode, on='user_id', how='left')
                        # merge 后列应该存在，填充缺失值
                        if col in data.columns:
                            data[col] = data[col].fillna(0)  # 填充缺失值
                        else:
                            # 如果 merge 失败（不应该发生），创建列并填充为 0
                            print(f"Warning: Column {col} not found after merge, creating with default value 0")
                            data[col] = 0
                        user_aux_features.append(col)
    
    # 物品特征：来自物品信息表，用于区分不同物品（特别是新物品）
    item_aux_features = []
    if use_aux_features and item_info_df is not None:
        # 合并物品特征
        item_feature_cols = ['category_id']  # words_count 和 created_at_ts 是连续特征，需要特殊处理
        for col in item_feature_cols:
            if col in item_info_df.columns:
                data = data.merge(item_info_df[['click_article_id', col]], 
                                on='click_article_id', how='left')
                data[col] = data[col].fillna(-1).astype(int)  # 填充缺失值（-1表示未知）
                item_aux_features.append(col)
        
        # 处理连续特征（words_count, created_at_ts）：需要归一化和分桶
        # 这里先简化处理，只添加 category_id，后续可以扩展
        # 如果需要添加连续特征，可以使用分桶或直接使用 DenseFeat
    ...
    for feature in features:
        ...
        if feature in item_aux_features and feature == 'category_id':
            # category_id 的 -1 表示未知/新物品，需要特殊处理
            unique_values = sorted(data[feature].unique())
            if -1 in unique_values:
                unique_values.remove(-1)
                unique_values = [-1] + unique_values  # -1 放在最前面，编码为 0
        ...
        # 为 OOV 预留一个索引（除了 category_id，它的 -1 已经是 OOV）
        if feature == 'category_id':
            feature_max_idx[feature] = data[feature].max() + 1  # category_id 的 -1 已经是 OOV
        else:
    ...
    # 3. 对于新用户/物品（OOV）: 使用 feature_max_idx[feature] - 1 作为 OOV 索引（使用默认 embedding）
    ...
    print(f"LabelEncoder 已保存到 {save_path}youtube_label_encoders.pkl")
    print(f"Vocabulary size 已保存到 {save_path}youtube_feature_max_idx.pkl")
    print(f"\n使用的辅助特征：")
    print(f"  用户特征（用户塔）: {user_aux_features}")
    print(f"  物品特征（物品塔）: {item_aux_features}")
    print(f"\n⚠️ 重要提示：")
    print(f"1. 如果在采样数据上训练，只能对采样中的用户/物品进行召回")
    print(f"2. 对于新用户/新物品，现在可以通过辅助特征（用户特征和物品特征）来区分")
    print(f"3. 用户特征和物品特征在双塔模型中保持独立，不会混合")
    ...
    # 将数据整理成模型可以直接输入的形式
    # 用户塔特征：user_id + 用户辅助特征 + 历史序列（保持独立）
    user_feature_columns = [SparseFeat('user_id', feature_max_idx['user_id'], embedding_dim)]
    # 添加用户辅助特征到用户塔
    for feat in user_aux_features:
        user_feature_columns.append(SparseFeat(feat, feature_max_idx[feat], embedding_dim))
    # 添加历史序列特征
    ...
    # 物品塔特征：click_article_id + 物品辅助特征（保持独立）
    ...
    for feat in item_aux_features:
        ...
        for item_id in item_profile['click_article_id']:
            ...
            if len(item_data) > 0:
                ...
                # 如果找不到，使用 OOV 索引（0 或 feature_max_idx[feat] - 1）
    ...
    # model.user_input 的顺序：['user_id:0', 'hist_article_id:0', 'hist_len:0', ...辅助特征...]
    # 对应的字典键：['user_id', 'hist_article_id', 'hist_len', ...辅助特征...]
    def get_feature_data(input_layer, input_dict):
        ...
        if feature_name not in input_dict:
            # 尝试从 test_user_model_input 或 all_item_model_input 中获取 batch_size
        ...
        # click_article_id 和辅助特征（用户特征、物品特征）应该是 (batch_size, 1) 的形状
...
# 传入 item_info_df 和 all_click_df 以支持辅助特征，帮助处理新用户/新物品冷启动
# 注意：all_click_df 需要包含用户特征（click_environment, click_os 等）
if not metric_recall:
    ...
    user_multi_recall_dict['youtubednn_recall'] = youtubednn_u2i_dict(all_click_df, 
    ...
    try:
        ...
        all_click_with_features = all_click_df
    user_multi_recall_dict['youtubednn_recall'] = youtubednn_u2i_dict(trn_hist_click_df, 
                                                                      item_info_df=item_info_df, 
                                                                      all_click_df=all_click_with_features,
                                                                      topk=20, 
                                                                      use_aux_features=True)
...
print(f"\n=== ItemCF 召回开始 ===")
print(f"metric_recall = {metric_recall}")
print(f"all_click_df 形状: {all_click_df.shape}")

if metric_recall:
    trn_hist_click_df, trn_last_click_df = get_hist_and_last_click(all_click_df)
else:
    trn_hist_click_df = all_click_df
    print(f"metric_recall=False, 使用全量数据: trn_hist_click_df 形状 = {trn_hist_click_df.shape}")
