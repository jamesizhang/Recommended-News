...
# from news3 import all_click_df
...
def trian_item_word2vec(click_df, embed_size=64, save_name='item_w2v_emb.pkl', split_char=' '):
    ...
    # 注意：新版本 gensim 中 size 改为 vector_size，iter 改为 epochs
    w2v = Word2Vec(docs, vector_size=16, sg=1, window=5, seed=2020, workers=24, min_count=1, epochs=50)

    # 保存成字典的形式
    # 注意：新版本 gensim 中需要使用 w2v.wv[key] 而不是 w2v[key]
...
all_click_df = get_all_click_df(offline=False, sample_size=20000)
...
# 修复：
# 1. 提高采样率和最大限制，确保每个用户有足够的负样本用于训练
# 2. 过滤掉没有正样本的用户（只有负样本的用户无法学习排序）
# 3. 控制正负样本比例在合理范围内（1:5 到 1:10）
# ⚠️ 注意：负采样是随机的，每次运行结果可能不同，这可能导致特征文件不一致
def neg_sample_recall_data(recall_items_df, sample_rate=0.2, max_neg_per_user=20, min_pos_per_user=1, random_seed=42):
    """
    负采样函数
    
    Args:
        recall_items_df: 召回数据 DataFrame
        sample_rate: 负采样率
        max_neg_per_user: 每个用户最多采样的负样本数
        min_pos_per_user: 每个用户最少需要的正样本数
    ...
    print('pos_data_num:', len(pos_data), 'neg_data_num:', len(neg_data), 'pos/neg:', len(pos_data) / len(neg_data) if len(neg_data) > 0 else 0)
    
    # 检查没有正样本的用户
    users_with_pos = set(pos_data['user_id'].unique())
    users_without_pos = set(neg_data['user_id'].unique()) - users_with_pos
    print(f'有正样本的用户数: {len(users_with_pos)}')
    print(f'没有正样本的用户数: {len(users_without_pos)}')
    
    if len(users_without_pos) > 0:
        print(f'⚠️  警告：{len(users_without_pos)} 个用户没有正样本（这些用户将被过滤）')
        # 过滤掉没有正样本的用户（这些用户无法学习排序）
        neg_data = neg_data[neg_data['user_id'].isin(users_with_pos)]

    # 分组采样函数
    def neg_sample_func(group_df):
        neg_num = len(group_df)
        # 提高采样率：从 0.001 提高到 0.2（20%），确保有足够的负样本
        # 对于28个负样本，采样后会有 min(max(int(28*0.2), 1), max_neg_per_user) = 5 个负样本
        sample_num = max(int(neg_num * sample_rate), 1)  # 保证最少有一个
        # 提高最大限制：从 5 提高到 max_neg_per_user，确保每个用户有足够的样本用于训练
        sample_num = min(sample_num, max_neg_per_user)  # 保证最多不超过 max_neg_per_user 个
        return group_df.sample(n=sample_num, replace=True, random_state=random_seed)
    ...
    # ⚠️ 诊断：检查负采样后的样本分布
    print(f'\n[诊断] 负采样后的统计：')
    print(f'  采样前总样本数: {len(recall_items_df)}')
    print(f'  采样后总样本数: {len(data_new)}')
    print(f'  采样后正样本数: {len(data_new[data_new["label"] == 1])}')
    print(f'  采样后负样本数: {len(data_new[data_new["label"] == 0])}')
    print(f'  采样后用户数: {data_new["user_id"].nunique()}')
    print(f'  采样后用户-物品对数: {len(data_new)}')
    print(f'  采样后唯一用户-物品对数: {len(data_new.drop_duplicates(["user_id", "sim_item"]))}')
...
# 获取热门物品（从训练数据中统计）
try:
    all_click_for_hot = pd.concat([click_trn, click_tst], ignore_index=True) if click_tst is not None else click_trn
    hot_items = all_click_for_hot['click_article_id'].value_counts().index.tolist()
...
if len(hot_items) > 0:
    ...
    for user_id, recall_list in recall_list_dict.items():
        # 处理不同的召回结果格式
        if isinstance(recall_list, dict):
            recall_items = list(recall_list.keys())
        elif isinstance(recall_list, list):
            # 如果是列表，提取物品ID
            recall_items = []
            for item in recall_list:
                if isinstance(item, (list, tuple)) and len(item) >= 1:
                    recall_items.append(item[0])
                else:
                    recall_items.append(item)
        else:
            recall_items = []
        
        if len(recall_items) < min_items_per_user:
            users_need_fill.append(user_id)
            # 获取用户已有的物品
            existing_items = set(recall_items)
            
            # 从热门物品中选择未包含的物品进行填充
            needed_count = min_items_per_user - len(recall_items)
            filled_items = []
            ...
            for hot_item in hot_items:
                if hot_item not in existing_items:
                    ...
                    if isinstance(recall_list, dict):
                        # 如果是字典格式，使用较低的分数
                        min_score = min(recall_list.values()) if len(recall_list) > 0 else 0.0
                        recall_list_dict[user_id][hot_item] = min_score - 0.1
                    elif isinstance(recall_list, list):
                        # 如果是列表格式，添加 (item, score) 元组
                        min_score = min([item[1] for item in recall_list if isinstance(item, (list, tuple)) and len(item) >= 2], default=0.0)
                        recall_list_dict[user_id].append((hot_item, min_score - 0.1))
                    filled_items.append(hot_item)
...
# 辅助函数：处理 groupby().apply() 的结果（可能是 Series 或 DataFrame）
def process_groupby_result(groupby_result):
    """统一处理 groupby().apply() 的结果，转换为标准的 DataFrame"""
    if isinstance(groupby_result, pd.Series):
        # 如果是 Series，转换为 DataFrame 然后 reset_index
        return groupby_result.to_frame(name=0).reset_index()
    else:
        # 如果是 DataFrame，直接 reset_index
        # 注意：如果 DataFrame 中已有 user_id 列，reset_index() 会报错，所以先删除
        ...
        if 0 not in result.columns:
            # 如果 reset_index() 后没有 0 列，将第一列（除了 user_id）重命名为 0
...
trn_user_item_label_tuples = process_groupby_result(trn_user_item_label_df.groupby('user_id').apply(make_tuple_func))
trn_user_item_label_tuples_dict = dict(zip(trn_user_item_label_tuples['user_id'], trn_user_item_label_tuples[0]))

if val_user_item_label_df is not None:
    val_user_item_label_tuples = process_groupby_result(val_user_item_label_df.groupby('user_id').apply(make_tuple_func))
    val_user_item_label_tuples_dict = dict(zip(val_user_item_label_tuples['user_id'], val_user_item_label_tuples[0]))
else:
    val_user_item_label_tuples_dict = None

tst_user_item_label_tuples = process_groupby_result(tst_user_item_label_df.groupby('user_id').apply(make_tuple_func))
tst_user_item_label_tuples_dict = dict(zip(tst_user_item_label_tuples['user_id'], tst_user_item_label_tuples[0]))

# 调试信息：测试集用户覆盖检查
print('[CHK] tst_user_item_label_tuples_dict users:', len(tst_user_item_label_tuples_dict))
try:
    _tst_users_from_dict = set(tst_user_item_label_tuples_dict.keys())
    _tst_users_from_hist = set(click_tst_hist['user_id'].unique())
    print('[CHK] click_tst_hist users:', len(_tst_users_from_hist))
    print('[CHK] intersection users:', len(_tst_users_from_dict & _tst_users_from_hist))
except Exception as _e:
    print('[CHK] user coverage check error:', _e)
...
# 调试信息：准备构建测试集特征
print('[CHK] before create_feature (test), users to build:', len(tst_user_item_label_tuples_dict.keys()))
tst_user_item_feats_df = create_feature(tst_user_item_label_tuples_dict.keys(), tst_user_item_label_tuples_dict, \
                                        click_tst_hist, article_info_df, item_content_emb_dict)
print('[CHK] tst_user_item_feats_df shape:', tst_user_item_feats_df.shape)
try:
    print('[CHK] tst_user_item_feats_df head:\n', tst_user_item_feats_df.head())
except Exception:
    pass

# 保存一份省的每次都要重新跑，每次跑的时间都比较长
if tst_user_item_feats_df.shape[0] == 0:
    raise ValueError('[BUILD] tst_user_item_feats_df is empty. Check recall/test coverage in news4.py')
...
def _calc_is_cat_hab(df: pd.DataFrame) -> pd.Series:
    """计算用户是否偏好该文章类别的标记，返回 0/1 的 Series。"""
    result = []
...
trn_user_item_feats_df['is_cat_hab'] = _calc_is_cat_hab(trn_user_item_feats_df)
if val_user_item_feats_df is not None:
    val_user_item_feats_df['is_cat_hab'] = _calc_is_cat_hab(val_user_item_feats_df)
else:
    val_user_item_feats_df = None
tst_user_item_feats_df['is_cat_hab'] = _calc_is_cat_hab(tst_user_item_feats_df)
